...
@if($showSensitiveDataModal && $sensitiveDataFindings)
    <x-beartropy-ui::modal
        wire:model="showSensitiveDataModal"
        styled
        max-width="md"
    >
        <x-slot:title>
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-start gap-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                ...
                <p class="text-sm text-amber-800 dark:text-amber-200">
                    <strong>{{ $sensitiveDataCollectionName }}</strong> contains values that look like credentials or secrets. Synced data is stored as <strong>readable YAML</strong> in your Git repository.
                </p>
            </div>
            ...
            {{-- Findings List --}}
            <div class="max-h-64 overflow-y-auto beartropy-thin-scrollbar space-y-3">
                @php
                    $grouped = collect($sensitiveDataFindings)->groupBy(fn ($f) => $f['request_name'] ?? 'Collection Variables');
                @endphp

                @foreach($grouped as $group => $items)
                    <div>
                        <h4 class="text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">{{ $group }}</h4>
                        <div class="space-y-1">
                                ...
                                <div class="flex items-center gap-2 px-2.5 py-1.5 rounded-md bg-gray-50 dark:bg-gray-800/50 text-xs">
                                    @php
                                        $badgeColors = match($finding['source']) {
                                            ...
                                            'variable' => 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400',
        ...
        <x-slot:footer>
            <div class="flex items-center justify-end gap-2">
                <x-beartropy-ui::button tint wire:click="closeSensitiveDataModal" sm>
                    Cancel
                </x-beartropy-ui::button>
                <x-beartropy-ui::button tint wire:click="confirmEnableSyncWithSensitiveData" sm>
                    Sync Anyway
                </x-beartropy-ui::button>
                <x-beartropy-ui::button
                    wire:click="confirmEnableSyncSanitized"
                    sm
